...
// Prefer PNG icon for notifications (SVGs may fail on some platforms). If the
// PNG is missing the create call will fall back to no icon.
const NOTIF_ICON_PNG = chrome.runtime.getURL('icons/icon48.png');
const NOTIF_ICON_SVG = chrome.runtime.getURL('icons/icon48.svg');
...
function createNotification(options, cb) {
  ...
  try {
    ...
    console.log('🔔 createNotification options (attempt PNG):', base);

    chrome.notifications.create(base, (id) => {
      ...
      console.warn('createNotification(PNG) failed:', err);

      // If PNG failed to download (common on some platforms), try SVG
      if (err.includes('Unable to download all specified images') || err.includes('Failed to download')) {
        const svgOpts = Object.assign({}, base, { iconUrl: NOTIF_ICON_SVG });
        console.log('🔔 createNotification options (attempt SVG):', svgOpts);
        chrome.notifications.create(svgOpts, (id2) => {
          if (!chrome.runtime.lastError) {
            if (cb) cb(id2);
            return;
          }
          console.warn('createNotification(SVG) failed:', chrome.runtime.lastError && chrome.runtime.lastError.message);
          // last resort: create without icon
          const noIcon = Object.assign({}, base, { iconUrl: undefined });
          delete noIcon.iconUrl;
          console.log('🔔 createNotification options (attempt no-icon):', noIcon);
          chrome.notifications.create(noIcon, (id3) => { if (cb) cb(id3); });
        });
      } else {
        // Some other error — still try without icon
        const noIcon2 = Object.assign({}, base, { iconUrl: undefined });
        delete noIcon2.iconUrl;
        console.log('🔔 createNotification options (attempt no-icon fallback):', noIcon2);
        chrome.notifications.create(noIcon2, (id4) => { if (cb) cb(id4); });
      }
    });
  } catch (e) {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  const deviceToken = local?.device_token;
  ...
  try {
    ...
    if (res.ok) {
      ...
      // refresh their UI.
      const viewUrl = `${DASHBOARD_URL}/dashboard`;
      const inlinePayload = {
        type: 'ITEM_SAVED',
        item: data,
        inlineToast: {
  ...
  createNotification({ type: 'basic', title: 'MindFlow Extension', message: '⚠️ Session expired. Please sign in on MindFlow web app to re-link.', priority: 2, requireInteraction: true, buttons: [{ title: 'Open Dashboard' }] }, (id) => { console.log('🔔 notification.create callback', id, chrome.runtime.lastError && chrome.runtime.lastError.message); if (!chrome.runtime.lastError) notificationData[id] = { expired: true, timestamp: Date.now() }; });
    ...
    // Show a developer notification so it's obvious in UI when debugging
    try {
  console.log('🔔 Creating failure notification');
  createNotification({ type: 'basic', title: 'MindFlow (Save failed)', message: `❌ ${e.message}`.slice(0, 200), priority: 1 }, (id) => { console.log('🔔 notification.create callback', id, chrome.runtime.lastError && chrome.runtime.lastError.message); });
    } catch (n) {
...
chrome.notifications.onClicked.addListener((id) => { chrome.tabs.query({ url: `${DASHBOARD_URL}/*` }, (tabs) => { if (tabs.length > 0) { chrome.tabs.update(tabs[0].id, { active: true }); chrome.windows.update(tabs[0].windowId, { focused: true }); } else chrome.tabs.create({ url: `${DASHBOARD_URL}/dashboard` }); }); chrome.notifications.clear(id); if (notificationData[id]) delete notificationData[id]; });

chrome.notifications.onButtonClicked.addListener((id) => { chrome.tabs.query({ url: `${DASHBOARD_URL}/*` }, (tabs) => { if (tabs.length > 0) { chrome.tabs.update(tabs[0].id, { active: true }); chrome.windows.update(tabs[0].windowId, { focused: true }); } else chrome.tabs.create({ url: `${DASHBOARD_URL}/dashboard` }); }); chrome.notifications.clear(id); if (notificationData[id]) delete notificationData[id]; });
